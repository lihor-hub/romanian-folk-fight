...
fn run_journey(
    ...
    wait_for_screen(&checkpoint, "Town")
        .map_err(|error| smoke(viewport, "town hub", error, &phase_dir))?;
    send_command(
        &checkpoint,
        serde_json::json!({"cmd": "pressButton", "button": "TownArena"}),
    )
    .map_err(|error| smoke(viewport, "enter arena", error, &phase_dir))?;
    ...
    .map_err(|error| smoke(viewport, "continue to town", error, &phase_dir))?;
    wait_for_screen(&checkpoint, "Town")
        .map_err(|error| smoke(viewport, "town hub", error, &phase_dir))?;
    send_command(
        &checkpoint,
    ...
    wait_for_screen(&checkpoint, "Town")
        .map_err(|error| smoke(viewport, "town after shop", error, &phase_dir))?;
    send_command(
        &checkpoint,
        serde_json::json!({"cmd": "pressButton", "button": "TownArena"}),
    )
