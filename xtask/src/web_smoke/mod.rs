...
/// `accessibility-settings-reload` (#191), `reduced-motion-fight` (#200), and
/// `fight-palette-desktop` (#189) -- each one the exact extension pattern
/// the module docs above describe: a new module plus one match arm here,
/// nothing else touched upstream.
pub fn run_scenario(scenario: &str, update_baselines: bool) -> Result<(), SmokeError> {
    match scenario {
        ...
        other => Err(SmokeError::usage(format!(
            "unknown --scenario `{other}` (known scenarios: cold-menu, gold-journey, accessibility-settings-reload, reduced-motion-fight, fight-palette-desktop)"
        ))),
    }
}
