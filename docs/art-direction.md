...
- Mask channels are positional and must match the catalog `palette` order: red
  ...
  reserved for the unchanged albedo silhouette, so representative records use
  no fourth palette entry. In the known-good Romanian set, red exposes skin or
...
`scripts/generate-human-material-channels.py`; its internal visual-reference
prompt and source are recorded in `.superpowers/sdd/task-3-report.md`. The
reference guides palette, material identity, and lighting only. It is not
runtime art and is never used to redraw the accepted albedo silhouettes.
